...
function applyReceiptReaderCandidates(reader) {
  ...
  const candidates = (Array.isArray(result.items) ? result.items : []).filter((item) => item?.productName && Number.isFinite(Number(item.amount)) && Number(item.amount) >= 0).map((item, index) => ({ id: `reader-${provider}-${index + 1}`, lineOrder: index + 1, productName: item.productName, quantity: item.quantity == null ? 1 : item.quantity, unitPrice: item.unitPrice == null ? item.amount : item.unitPrice, amount: item.amount, category: ITEM_CATEGORY_OPTIONS[ITEM_CATEGORY_OPTIONS.length - 1], purpose: { value: '', source: 'ocr', confidence: 0 }, submissionStatus: 'review', source: 'ocr', confidence: item.confidence, basis: [`${provider} receipt reader`, ...(item.sourceText ? [`sourceText: ${item.sourceText}`] : [])], sourceLines: item.sourceText ? [item.sourceText] : [], sourceLineNumbers: [], notes: '' }));
  cachedOcrCandidates = candidates; ocrQuality = result.quality || { status: 'low_confidence', candidateCount: candidates.length, lowConfidenceCount: candidates.filter((item) => item.confidence == null || item.confidence < .65).length, receiptTotalAmount: result.receiptTotalAmount ?? null, itemTotalAmount: candidates.reduce((sum, item) => sum + Number(item.amount || 0), 0), difference: null, totalConsistent: false };
  if (shouldAutofillOcrCandidates(items, candidates)) { items = candidates.map((item, index) => createReceiptItem({ ...item, lineOrder: index + 1 })); render(); return { candidateCount: candidates.length, added: candidates.length, retained: false, headers: { vendor: result.vendor || '', purchaseDate: result.purchaseDate || '', receiptTotalAmount: result.receiptTotalAmount ?? null }, quality: ocrQuality }; }
  render(); return { candidateCount: candidates.length, added: 0, retained: items.length > 0, headers: { vendor: result.vendor || '', purchaseDate: result.purchaseDate || '', receiptTotalAmount: result.receiptTotalAmount ?? null }, quality: ocrQuality };
}
function install() {
