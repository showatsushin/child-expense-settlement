...
publicConfig.receiptReaderProvider = process.env.RECEIPT_READER_PROVIDER || 'tesseract';
await writeFile(resolve(dist, 'config.js'), `globalThis.__APP_CONFIG__ = ${JSON.stringify(publicConfig)};`);
for (const file of ['index.html', 'styles.css', 'phase2.js', 'mobile-ui.js', 'reason-suggestions.js', 'receipt-items.js', 'evidence-export.js', 'favicon.ico', 'favicon-16x16.png', 'favicon-32x32.png', 'apple-touch-icon.png', 'icon-192x192.png', 'icon-512x512.png']) await copy(file);
await copy('src');
