...
import { formatSubmissionEvidenceNumber } from './src/submission-evidence-number.js';
...
function settlementMarkup(record, evidence) {
  ...
  return `<section class="submission-settlement"><h1>${esc(submissionNumber(evidence))} 清算資料</h1><p>内部管理番号: ${esc(evidence.evidenceNumber)}</p><dl><div><dt>購入日</dt><dd>${esc(record.paidDate)}</dd></div><div><dt>購入店</dt><dd>${esc(record.vendor?.value ?? record.vendor)}</dd></div><div><dt>レシート総額</dt><dd>${yen(receiptTotal(record))}</dd></div><div><dt>提出対象額</dt><dd>${yen(receiptClaimTotal(record))}</dd></div></dl><table><thead><tr><th>商品名</th><th>種別</th><th>金額</th><th>購入目的・必要性</th></tr></thead><tbody>${itemRows || '<tr><td colspan="4">購入品明細はありません。</td></tr>'}</tbody></table></section>`;
}
