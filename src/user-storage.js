...
export function createUserStorage(userId) {
  ...
  const root = namespaceForUser(userId); const keys = { records:`${root}records.v2`, evidences:`${root}evidences.v2`, children:`${root}children.v2`, schema:`${root}schemaVersion` };
  ...
  return {
    ...
    saveMigratedState: (state) => { write(keys.records,state.records);write(keys.evidences,state.evidences);write(keys.children,state.children);localStorage.setItem(keys.schema,String(SCHEMA_VERSION)); }, saveFile, getFile, deleteFile,
  };
}
