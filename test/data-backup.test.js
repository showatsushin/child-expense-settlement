...
import { strFromU8, unzipSync, zipSync } from '../node_modules/fflate/esm/browser.js';
...
function userStorage({ records = [], evidences = [], children = [], confirmedHistory = {}, files = [], schemaVersion = APP_SCHEMA_VERSION } = {}) {
  let state = { records:clone(records), evidences:clone(evidences), children:clone(children), confirmedHistory:clone(confirmedHistory), schemaVersion };
  ...
  return {
    ...
    replaceBackupState: async (next) => { replaceCalls += 1; state = { records:clone(next.records), evidences:clone(next.evidences), children:clone(next.children), confirmedHistory:clone(next.confirmedHistory), schemaVersion:next.schemaVersion }; blobs = new Map(next.files.map((entry) => [entry.id, entry.file])); },
...
function fixture() {
  const evidence = { id:'ev_keep', evidenceNumber:'E-001', submissionEvidenceNumber:{ prefix:'甲', number:1, subNumber:null }, fileName:'IMG_1234.JPG', mimeType:'image/jpeg', size:3, createdAt:'2026-09-14T10:00:00.000Z', status:'attached', ocr:{ status:'completed', rawText:'raw', correctedText:'corrected' } };
  const item = { id:'item_keep', receiptId:'record_keep', lineOrder:1, productName:'water', quantity:1, unitPrice:120, amount:120, taxRate:'8', amountInputMode:'tax_included', category:'飲料水', purpose:{ value:'水分補給', source:'manual_override', confidence:null }, submissionStatus:'included', knowledgeKey:'drinking_water', knowledgeSource:'knowledge', knowledgeVersion:2, purposeSource:'manual_override', categorySource:'manual_override', originalKnowledgePurpose:'元の目的', notes:'edited', createdAt:'2026-09-14T10:00:00.000Z', updatedAt:'2026-09-14T10:05:00.000Z' };
  ...
  return { records:[record], evidences:[evidence], children:[{ id:'child_keep', name:'子1' }], confirmedHistory:{ taxRateHistory:[{ sourceValue:'water', confirmedValue:'water', normalizedKey:'water', confirmedTaxRate:'8', createdAt:'2026-09-14T10:00:00.000Z', updatedAt:'2026-09-14T10:00:00.000Z', useCount:1, source:'user_confirmed_history' }] }, files:[{ id:'ev_keep', file:new Blob([new Uint8Array([1,2,3])], { type:'image/jpeg' }) }] };
}
...
test('backup contains only the active storage data, preserves receipt items and does not mutate it', async () => {
  ...
  assert.equal(parsed.records[0].id, 'record_keep'); assert.equal(parsed.records[0].items[0].id, 'item_keep'); assert.equal(parsed.records[0].items[0].knowledgeKey, 'drinking_water'); assert.equal(parsed.evidences[0].id, 'ev_keep'); assert.equal(parsed.confirmedHistory.taxRateHistory[0].confirmedTaxRate, '8'); assert.deepEqual(parsed.evidences[0].submissionEvidenceNumber, { prefix:'甲', number:1, subNumber:null }); assert.deepEqual(await bytes(parsed.files[0].file), [1,2,3]);
...
test('archive never serializes unrelated local data or secrets', async () => {
  ...
  assert.equal(JSON.stringify(entries).includes('never-back-up-this'), false); assert.deepEqual(Object.keys(entries).sort(), ['data/children.json','data/confirmed-history.json','data/evidence.json','data/originals.json','data/records.json','data/schema.json','manifest.json']);
});

test('v1 backup without history restores with an empty confirmed history', async () => {
  const archive = await createBackupArchive({ storage:userStorage(fixture()), appSchemaVersion:APP_SCHEMA_VERSION });
  const files = unzipSync(new Uint8Array(await archive.archive.arrayBuffer())); delete files['data/confirmed-history.json'];
  const parsed = await parseBackupArchive(new Blob([zipSync(files)]), { appSchemaVersion:APP_SCHEMA_VERSION });
  assert.deepEqual(parsed.confirmedHistory.taxRateHistory, []);
  assert.deepEqual(parsed.confirmedHistory.knowledgeSelectionHistory, []);
});
...
test('restore keeps IDs, references, user edits, originals, and validates counts', async () => {
  ...
  assert.deepEqual(restored, { recordCount:1, evidenceCount:1, originalFileCount:1, receiptItemCount:1 }); assert.equal(after.replaceCalls, 1); assert.equal(after.state.records[0].id, 'record_keep'); assert.equal(after.state.records[0].evidenceIds[0], 'ev_keep'); assert.equal(after.state.records[0].items[0].id, 'item_keep'); assert.equal(after.state.records[0].items[0].categorySource, 'manual_override'); assert.equal(after.state.records[0].items[0].taxRate, '8'); assert.equal(after.state.records[0].items[0].amountInputMode, 'tax_included'); assert.equal(after.state.confirmedHistory.taxRateHistory[0].confirmedTaxRate, '8'); assert.equal(after.state.evidences[0].fileName, 'IMG_1234.JPG'); assert.deepEqual(after.state.evidences[0].submissionEvidenceNumber, { prefix:'甲', number:1, subNumber:null }); assert.deepEqual(after.files[0].bytes, [1,2,3]);
});
