...
test('Excelデータ生成は内部管理番号と提出用証拠番号を別列で返す', () => { const data=buildWorkbookData([{id:'r',evidenceIds:['e'],amount:{value:100},childId:{value:'c'},category:{value:'医療費'}}],[{id:'e',evidenceNumber:'E-001',submissionEvidenceNumber:{prefix:'甲',number:1,subNumber:null},fileName:'a.jpg',ocr:{status:'completed'}}],[{id:'c',name:'子1'}]); assert.deepEqual(data.settlement[0].slice(0,2),['内部管理番号','提出用証拠番号']); assert.deepEqual(data.evidence[1].slice(0,2),['E-001','甲第1号証']); assert.equal(data.summary[1][1],100); });
